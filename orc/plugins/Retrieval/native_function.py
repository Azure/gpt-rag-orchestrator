from shared.util import get_aoai_config, extract_text_from_html, get_possitive_int_or_default
...
AZURE_OPENAI_CHATGPT_DEPLOYMENT = config.get_value("AZURE_OPENAI_CHATGPT_DEPLOYMENT")
AZURE_OPENAI_RESOURCE = config.get_value("AZURE_OPENAI_RESOURCE")
AZURE_OPENAI_TEMPERATURE = config.get_value("AZURE_OPENAI_TEMPERATURE", "0.17")
AZURE_OPENAI_APIVERSION = config.get_value("AZURE_OPENAI_API_VERSION")
AZURE_OPENAI_EMBEDDING_DEPLOYMENT = config.get_value("AZURE_OPENAI_EMBEDDING_DEPLOYMENT")
AZURE_OPENAI_EMBEDDING_APIVERSION = config.get_value("AZURE_OPENAI_EMBEDDING_APIVERSION")

# Azure Search Integration Settings
AZURE_SEARCH_SERVICE = config.get_value("AZURE_SEARCH_SERVICE")
AZURE_SEARCH_INDEX = config.get_value("AZURE_SEARCH_INDEX")
AZURE_SEARCH_API_VERSION = config.get_value("AZURE_SEARCH_API_VERSION", "2024-07-01")
...
AZURE_SEARCH_TOP_K = config.get_value("AZURE_SEARCH_TOP_K") or "3"
AZURE_SEARCH_USE_SEMANTIC = config.get_value("AZURE_SEARCH_USE_SEMANTIC") or "false"
AZURE_SEARCH_APPROACH = config.get_value("AZURE_SEARCH_APPROACH") or "hybrid"
AZURE_SEARCH_OYD_USE_SEMANTIC_SEARCH = config.get_value("AZURE_SEARCH_OYD_USE_SEMANTIC_SEARCH") or "false"
AZURE_SEARCH_OYD_USE_SEMANTIC_SEARCH = True if AZURE_SEARCH_OYD_USE_SEMANTIC_SEARCH == "true" else False
AZURE_SEARCH_SEMANTIC_SEARCH_CONFIG = config.get_value("AZURE_SEARCH_SEMANTIC_SEARCH_CONFIG") or "my-semantic-config"
AZURE_SEARCH_ENABLE_IN_DOMAIN = config.get_value("AZURE_SEARCH_ENABLE_IN_DOMAIN") or "true"
AZURE_SEARCH_ENABLE_IN_DOMAIN = True if AZURE_SEARCH_ENABLE_IN_DOMAIN == "true" else False
AZURE_SEARCH_CONTENT_COLUMNS = config.get_value("AZURE_SEARCH_CONTENT_COLUMNS") or "content"
AZURE_SEARCH_FILENAME_COLUMN = config.get_value("AZURE_SEARCH_FILENAME_COLUMN") or "filepath"
AZURE_SEARCH_TITLE_COLUMN = config.get_value("AZURE_SEARCH_TITLE_COLUMN") or "title"
AZURE_SEARCH_URL_COLUMN = config.get_value("AZURE_SEARCH_URL_COLUMN") or "url"

# Bing Search Integration Settings
BING_SEARCH_TOP_K = config.get_value("BING_SEARCH_TOP_K") or "3"
BING_CUSTOM_SEARCH_URL = "https://api.bing.microsoft.com/v7.0/custom/search?"
BING_SEARCH_MAX_TOKENS = config.get_value("BING_SEARCH_MAX_TOKENS") or "1000"
...
# APIM Settings
APIM_ENABLED = config.get_value('APIM_ENABLED', 'false').lower() == 'true'
APIM_BING_CUSTOM_SEARCH_URL = config.get_value('APIM_BING_CUSTOM_SEARCH_URL', "") + "/search?"
APIM_AZURE_SEARCH_URL = config.get_value('APIM_AZURE_SEARCH_URL', "")
...
class Retrieval:
    ...
    async def BingRetrieval(
        ...
        bing_custom_config_id = config.get_value('BING_CUSTOM_CONFIG_ID')
        if(APIM_ENABLED):
