...
- **Admin dashboard Configuration tab (refs [Azure/GPT-RAG#512](https://github.com/Azure/GPT-RAG/issues/512)):** The admin dashboard introduced in #511 gains a third tab — **Configuration** — that lets an authenticated admin view and edit a curated allowlist of App Configuration keys without leaving the orchestrator. The settings are grouped into five sections (Agent & generation, Conversation history, Retrieval & search, Reliability, Multimodal) and rendered with typed inputs (dropdown for `AGENT_STRATEGY` / `REASONING_EFFORT`, range+number for `CHAT_TEMPERATURE` / `CHAT_TOP_P`, number inputs for the token / message / top-k / retry counters, and toggles for the booleans). Every field exposes an accessible info popover — keyboard- and screen-reader-reachable, not hover-only — whose body is sourced from a single backend metadata module (`src/api/config_settings.py`) so docs and UI never drift. New endpoints on the dashboard router, all protected by `require_admin`: `GET /api/dashboard/config` returns the current values + metadata; `PUT /api/dashboard/config` validates type / range / enum / denylist and writes accepted values to App Configuration under the `gpt-rag-orchestrator` label; `POST /api/dashboard/config/refresh` rebuilds the in-process AppConfig cache; `POST /api/dashboard/config/apply` is a soft-restart that refreshes the cache and returns a clear status string — a hard container restart would require `azure-mgmt-appcontainers` plus extra RBAC and remains a follow-up. A defense-in-depth denylist rejects writes to any key matching sensitive suffixes (`_APIKEY`, `_SECRET`, `_PASSWORD`, `_CONNECTION_STRING`, `_TOKEN`, ...) or to an explicit set (`KEY_VAULT_URI`, `MCP_APP_APIKEY`, ...) even if a future refactor accidentally adds them to the allowlist. Validation errors come back as `422` with a per-key error list and surface inline in the UI without breaking the page; write errors come back as `500` with the same shape.

### Fixed
- **Restored `frontend/src/lib/api.ts` (fix for [#236](https://github.com/Azure/gpt-rag-orchestrator/pull/236)):** PR #236 merged the dashboard frontend without `lib/api.ts`, so a fresh `npm run build` in `frontend/` could not resolve the helpers imported by `App.tsx`, `OverviewTab.tsx`, `ConversationsTab.tsx`, and `ConversationDetailDialog.tsx`. This change reinstates the original module (typed wrapper around `fetch`, `ApiError`, `fetchVersion` / `fetchOverview` / `fetchConversations` / `fetchConversationDetail`, `formatUtc`) and extends it with the helpers used by the new Configuration tab. This is a bug fix for #236 carried in-scope here so the Configuration tab work can land with a green build.
